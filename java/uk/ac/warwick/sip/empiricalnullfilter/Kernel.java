 ...
 * kernel. The kernel can move right or start on a new row, the member variables are updated when
 * the kernel moves
 * <p>
 * <p>
 * Modified from <a
...
class Kernel {
  ...
  public void moveToNewLine(int y) {
    ...
    for (int i = 0; i < Kernel.getKHeight(); i++) {
      cachePointers[2 * i] = (i + this.y) * cache.getCacheWidth() + Kernel.getKRadius()
          + Kernel.getKernelPointer()[2 * i];
      cachePointers[2 * i + 1] = (i + this.y) * cache.getCacheWidth() + Kernel.getKRadius()
          + Kernel.getKernelPointer()[2 * i + 1];
    }
    ...
    if (this.isFinite) {
      ...
      if (this.isMeanStd) {
        ...
        if (this.nFinite < 2) {
          ...
          this.std = (float) Math.sqrt(
              ((this.sums[1] - this.sums[0] * this.sums[0] / this.nFinite) / (this.nFinite - 1)));
        }
      }
   ...
   * Output is written to array sums[0] = sum; sums[1] = sum of squares. Ignores nan. MODIFIES
   * nFinite.
   */
  ...
  /**
   * METHOD: GET QUARTILES Get the quartiles of values within kernel-sized neighborhood. nan values
   * are ignored.
   */
   ...
   *
   * <p>
   * kernelPointer: an array that gives the length of each line of the structuring element (kernel)
   * to the left (negative) and to the right (positive): [0] left in line 0, [1] right in line 0,
   * [2] left in line 2, ...
   *
   * <p>
   * Array elements at the end: length-2: nPoints, number of pixels in the kernel area length-1:
   * kernelRadius in x direction (kernel width is 2*kernelRadius+1)
   *
   * <p>
   ...
   *
   * <p>
   * Note that kernel width and height are the same for the circular kernels used here, but treated
   * separately for the case of future extensions with non-circular kernels.
   *
   * <p>
   ...
   * 
   * @param radius radius of the kernel. Radius = 0.5 includes the 4 neighbours of the pixel in the
   *        centre. Radius = 1 corresponds to a 3x3 kernel.
   */
  public static void setKernel(double radius) {
    if (radius >= 1.5 && radius < 1.75) {// this code creates the same sizes as the previous
                                         // RankFilters
      radius = 1.75;
