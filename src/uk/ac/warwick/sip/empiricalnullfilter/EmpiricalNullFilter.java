...
// EDITED RankFilters.java See
// https://github.com/imagej/ImageJA/blob/master/src/main/java/ij/plugin/filter/RankFilters.java
...
/**
 * Empirical Null Filter, locally normalise an image using mode and empirical null std
 *
 * Locally normalise the grey values using the empirical null mean (mode) and the empirical null
 * std.
 *
 * <p>
 * The method filter is overloaded but all are directed to filter() with no parameters. filter()
 * with no parameter is used directly by ImageJ because the image is passed through the methods
 * setup and run. The methods filter(float [][] image) and filter(float [][] image, String roiPath)
 * are required to pass the image when used by MATLAB.
 *
 * <p>
 * How to use:
 * <ul>
 * <li>Compile to a .jar file and use ImageJ or Fiji</li>
 * </ul>
 *
 * <p>
...
public class EmpiricalNullFilter implements ExtendedPlugInFilter, DialogListener {
  ...
  public static final String[] OUTPUT_NAME =
      {"null mean", "null std", "standard deviation", "quantile 1", "median", "quantile 3"};
  ...
  /**
   * the bandwidth parameter A where the bandwidth for the density estimate is (B x n^{-1/5} + A) *
   * std
   */
  protected float bandwidthParameterA = lastBandwidthParameterA;
  /**
   * the bandwidth parameter B where the bandwidth for the density estimate is (B x n^{-1/5} + A) *
   * std
   */
  ...
  /**
   * Setup of the PlugInFilter. Returns the flags specifying the capabilities and needs of the
   * filter.
   *
   * @param arg not used
   * @param ip not used
   ...
   *
   * @param pointer which output images to show, use and the static int variables, e.g. NULL_MEAN +
   *        NULL_STD
   */
  ...
  /**
   * Instantiate a cache (different cache can have paddings)
   */
   ...
   * unfiltered line. Usually, one thread reads reads several lines into the cache, while the others
   * are processing the data.
   *
   * <p>
   * 'aborted[0]' is set if the main thread has been interrupted (during preview) or ESC pressed.
   * 'aborted' must not be a class variable because it signals the other threads to stop; and this
   * may be caused by an interrupted preview thread after the main calculation has been started.
   */
   ...
   *
   * <p>
   * Data handling: The area needed for processing a line is written into the array 'cache'. This is
   * a strip of sufficient height for all threads to have each thread processing one line, and some
   * extra space if one thread is finished to start the next line. This array is padded at the edges
   * of the image so that a surrounding with radius kRadius for each pixel processed is within
   * 'cache'. Out-of-image pixels are set to NaN. When adding a new line, the lines in 'cache' are
   * not shifted but rather the smaller array with the start and end pointers of the kernel area is
   * modified to point at the addresses for the next line.
   *
   * <p>
   * Notes: For mean and variance, except for very small radius, usually do not calculate the sum
   * over all pixels. This sum is calculated for the first pixel of every line only. For the
  ...
  private void threadFilter(Cache cache, int[] yForThread, int threadNumber, int[] seeds,
    ...
    Kernel kernel =
        new Kernel(cache, roi, this.isKernelCopy, this.isKernelMeanVar, this.isKernelQuartile);
  ...
  protected float[] getNullMeanStd(float initialValue, Kernel kernel, NormalDistribution normal,
    ...
    try {
      empiricalNull = new EmpiricalNull(this.nInitial, this.nStep, this.log10Tolerance,
          this.bandwidthParameterA, this.bandwidthParameterB, initialValue, kernel, normal, rng);
  ...
  /**
   * Set number of Newton-Raphson steps
   *
   * @param nStep
   * @throws InvalidValueException
   */
  ...
  /**
   * Stops the newton-raphson algorithm when (Math.abs(dxLnF[1])&lt;tolerance) where dxLnF is the
   * first diff of the log density
   *
  ...
  /**
   * Set the bandwidth A parameter
   *
   * The bandwidth for the density estimate is bandwidthParameterB * Math.min(dataStd, iqr/1.34f)
   * ((float) Math.pow((double) this.n, -0.2)) + bandwidthParameterA;
   *
  ...
  void showMasks() {
    ...
    for (double r = 0.5; r < 50; r += 0.5) {
      ...
      for (int i = 0, y = y0; i < Kernel.getKHeight(); i++, y++)
        for (int x = w / 2 + Kernel.getKernelPointer()[2 * i],
            p = x + y * w; x <= w / 2 + Kernel.getKernelPointer()[2 * i + 1]; x++, p++)
          pixels[p] = 1f;
  ...
  /**
   * This method is called by ImageJ to set the number of calls to run(ip) corresponding to 100% of
   * the progress bar
   */
  @Override
